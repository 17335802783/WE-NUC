...
@WebServlet(name = "classTable", value = "/classTable")
public class ClassTableRequest extends HttpServlet {
    public void init() {
        String message = "Init!";
    }
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        System.out.println("classTable-响应消耗时间为:" + (tt - Long.parseLong(_t)) + "毫秒");
...
//        }
        try {
            ...
            ClassTable cls = usr.getUsr_classTable();

            if (cls == null) {
                out.println(JsonUtils.getJson(0, null, "获取失败"));
            } else {
                out.println(JsonUtils.getJson(1, cls.toJSONObject(), ""));
            }
            System.out.println(username + "，执行ClassTable（获取课程表）操作成功");
