...
@WebServlet(name = "tpz", value = "/tpz")
public class TrainingPlanRequestZyh extends HttpServlet {
...
//            usr.getUserInfo();
            JSONObject usl = usr.getTrainPlanZyh();
            if (usl == null) {
                out.println(JsonUtils.getJson(0, null, "获取失败"));
            } else {
                out.println(JsonUtils.getJson(1, usl, ""));
            }
            System.out.println(username + "，执行tpz（通过专业号获取培养计划）操作成功");
