...
@WebServlet(name = "execute", value = "/execute")
public class executePlanRequest extends HttpServlet {
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        try {
            ...
            JSON usl = usr.getExecutePlan();
            if (usl == null) {
                out.println(JsonUtils.getJson(0, null, "获取失败"));
            } else {
                out.println(JsonUtils.getJson(1, usl, ""));
            }
            System.out.println(username + "，执行executePlan（获取教学执行计划）操作成功");
